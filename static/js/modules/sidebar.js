...
export function renderSidebarTabs(sidebarTabs, chats, activeChatId, onSwitch, onDelete, closeSidebar) {
    ...
    chats.forEach(chat => {
        ...
        const displayTitle = chat.isVoice ? `[voice]-${chat.title}` : chat.title;
        tab.innerHTML = '<span>💬</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml(displayTitle) + '</span><span class="del-btn" style="opacity:0;background:none;border:none;color:#555;cursor:pointer;font-size:0.8rem;padding:2px 6px;">✕</span>';
